...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    test_flat = x_test.view(num_test, -1)
    for i in range(num_train):
        dists[i] = ((test_flat - train_flat[i]) ** 2).sum(dim = 1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_square = train_flat.pow(2).sum(-1, keepdim = True)
    test_square = test_flat.pow(2).sum(-1)
    cross = torch.matmul(train_flat, test_flat.T)
